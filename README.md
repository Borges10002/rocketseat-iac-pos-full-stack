 ...
 - AWS credentials configured (e.g., via `aws configure`, environment variables, or the setup script below)

 ## Configure AWS credentials

 This project uses the `stg` stack and the `us-east-2` AWS region. To configure AWS credentials directly in Pulumi as encrypted stack secrets, run:

 ```powershell
 .\scripts\configure-pulumi-aws.ps1
 ```

 The script prompts for `AWS Access Key ID`, `AWS Secret Access Key`, and an optional session token, then runs `pulumi preview --stack stg`.
